...
and place a PSIBLAST database under /data/psiblast and a uniclust database under /data/hhblits.
  ProSPr will download the database if it is missing (250GB and 25GB respectively).
'''

parser = argparse.ArgumentParser(description=desc_usg,
...
run_parser.add_argument('domain',
                        help = 'CASP13 target domain id' \
                        + '(to see list of available samples, run with any invalid char or string')

run_parser.add_argument('-n', '--network',
                        help = 'Which neural network do you want to use to make the predictions.' \
                        + 'ProSPr provides the following:  full, no-potts, and no-msa.  full is default',
                        default = 'full')

run_parser.add_argument('-s', '--stride',
                        help = 'stride over which crops of domain are predicted and averaged,' \
                        + 'integer 1-30.\nWARNING: Using a small stride may result in very long' \
                        + 'processing time! Suggested for quick prediction: 25',
                        type = int, default = 25)

train_parser = subparsers.add_parser('build')
train_parser.add_argument('domain',
                          help = 'Looks for /data/<domain>/<domain>.fasta for a sequence and' \
                          + 'produces a pkl file ProSPr can use.  Please note, gaps in the sequence' \
                          + 'are denoted by a -, not an X')
train_parser.add_argument('-j', '--threads',
                          help = 'Number of threads for HHBlits/PSI-BLAST runs',
                          default = 1, type = int)
...
def main(args):
    ...
    model_path = pconf.basedir
    model = load_model(model_path + args.network + ".nn") 
    #network = load_model("./data/" + "no-potts" + ".nn") 

    print(type(int(args.network)))
    dist_pred, dist_loss = domain(name = args.domain,
                                  model = model,
                                  stride = args.stride,
                                  network_type = int(args.network))
    
    #dist_pred, dist_loss = domain("T1016", "no-potts", 1)
...
def build(args):
    ...
    if p.simple_check_db() and p.decompressed():
        ...
        print("dude")
        res = input("Would you like to download the PSIBLAST database?' \
        + '(~250GB, this is required to use your own sequence) [y/n]")
        while True:
            if res == "y":
                p.download_db()
                p.decompress_db()
